...
const char* tesseraeKind() {
  ...
  // Same 800x480 SSD1677 panel as the X4, so the packed frame is byte-identical,
  // but it is a different product and gets its own catalog entry rather than
  // claiming to be an Xteink.
  return TESSERAE_GRAYSCALE ? "seeed_sticky_gray" : "seeed_sticky";
#elif FREEINK_DEVICE_X4PRO
  return TESSERAE_GRAYSCALE ? "xteink_x4_pro_gray" : "xteink_x4_pro";
#else
  if (gpio.deviceIsX3()) {
