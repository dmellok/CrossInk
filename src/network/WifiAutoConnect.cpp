...
bool WifiAutoConnect::connect(const uint32_t perNetworkTimeoutMs) {
  ...
  // The store loads itself on first access, so this works on a boot where the
  // user never opened the WiFi screen.
  ...
  if (!lastSsid.empty()) {
    if (const auto preferred = WIFI_STORE.findCredential(lastSsid)) {
